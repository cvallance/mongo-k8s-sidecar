...
const getK8sMongoPodLabels = () => process.env.KUBERNETES_POD_LABELS || process.env.MONGO_SIDECAR_POD_LABELS || false;
 ...
 */
const isConfigRS = () => {
  const configSvr = (process.env.MONGO_CONFIG_SVR || process.env.CONFIG_SVR || '').trim().toLowerCase();
...
module.exports = {
  k8sNamespace: process.env.KUBERNETES_NAMESPACE || process.env.KUBE_NAMESPACE,
  ...
  loopSleepSeconds: process.env.SIDECAR_SLEEP_SECONDS || process.env.MONGO_SIDECAR_SLEEP_SECONDS || 5,
  unhealthySeconds: process.env.SIDECAR_UNHEALTHY_SECONDS || process.env.MONGO_SIDECAR_UNHEALTHY_SECONDS || 15,
